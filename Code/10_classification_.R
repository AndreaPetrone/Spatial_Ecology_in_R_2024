# classification today and import data
# it is useful to understand the class of the object (one pixel is snow, other is wood etc) 
# once you know the class you have a map of all the pixel that are part of the classes
# you can calculate the amount of pixels per class

# We can go to a site, download an image, import to R and work on it

# Classification prosess in R using imageRy 
sun <- im.import("Solar_Orbiter_s_first_views_of_the_Sun_pillars.jpg") 

# We can classify this image and we specify the number of classes (or clusters)   

sunc <- im.classify(sun, num_clusters=3)  # the number of the classes (clusters, labels) might be different.

# matogrosso area to see forest vs non forest areas
...
m1992c <- im.classify(m1992, num_clusters=2) # 2 bc it's forest vs no-forest
...
# we need a package ggplot2, aes = aestethic, related to what u put in X, Y and final colour
# geom = what type of graph u want to put in it
# stat = type of statistic u want to use (we want just to use the data)
# fill = is the colour
